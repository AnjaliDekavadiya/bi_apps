...
class AccountReport(models.Model):
    ...
    def _data_real_time(self, options, workbook, worksheet, product, row):
        ...
        if total_cost:
            last_price = total_cost / total_qty
        for move in moves:
            ...
            if svl:
                ...
                worksheet.write_string('C' + str(row), move.picking.l10n_latam_document_number.split('-')[0] if move.picking.l10n_latam_document_number else (move.picking_id.serie or ''), format_txt)
                worksheet.write_string('D' + str(row), move.picking.l10n_latam_document_number.split('-')[1] if move.picking.l10n_latam_document_number else (move.picking_id.number or ''), format_txt)
                ...
                slcls = self.env['stock.valuation.adjustment.lines'].search([('move_id','=', move.id)])
                if slcls and not debug:
                    svl['value'] += sum(slcls.mapped('additional_landed_cost'))
                if move.picking_id.picking_type_code == 'incoming':
                    ...
                    if move.origin_returned_move_id:
                        ...
                        worksheet.write_number('J' + str(row), last_price, format_line_num)
                        worksheet.write_number('K' + str(row), abs(svl['quantity']) * last_price, format_line_num)
                        final_out_cost += abs(svl['quantity']) * last_price
                        final_out_qty += abs(svl['quantity'])
                    cost = svl['quantity'] * last_price
                ...
                if slcls and debug:
                    for slcl in slcls:
                        worksheet.write_string('D' + str(row), slcl.cost_id.name or '', format_txt)
                        worksheet.write_number('H' + str(row), slcl['additional_landed_cost'], format_line_num)
                        final_in_cost += slcl['additional_landed_cost']
                        total_cost += slcl['additional_landed_cost']
                        worksheet.write_number('L' + str(row), total_qty, format_line_qty)
                        worksheet.write_number('M' + str(row), 0 if total_qty == 0 else total_cost / total_qty, format_line_num)
                        worksheet.write_number('N' + str(row), total_cost, format_line_num)
                        last_price = 0 if total_qty == 0 else total_cost / total_qty
    ...
    def _get_kardex_initial_balance_real_time(self, options, workbook, worksheet, product, row):
        ...
        slcls = self.env['stock.valuation.adjustment.lines'].search([('move_id','in', moves.ids)])
        if slcls:
            total_cost += sum(slcls.mapped('additional_landed_cost'))
        if svls:
            for svl in svls:
