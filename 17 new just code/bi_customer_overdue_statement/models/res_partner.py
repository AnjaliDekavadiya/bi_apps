...
class Res_Partner(models.Model):
	...
	supplier_invoice_ids = fields.One2many('account.move', 'partner_id', 'Supplier move lines', domain=[('move_type', 'in', ['in_invoice','in_refund','entry']),('state', 'in', ['posted']),('is_set_statments', '=', True)])
	balance_invoice_ids = fields.One2many('account.move', 'partner_id', 'Customer move lines',domain=[('move_type', 'in', ['out_invoice','out_refund','entry']),('state', 'in', ['posted']),('is_set_statments', '=', True)])
	...
	monthly_payment_amount_due_amt = fields.Monetary(compute='_get_amounts_and_date_amount', string=" Filter Monthly Balance Due",store=True)
	...
	sup_monthly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
												  string=" Filter Supplier Monthly Balance Due")
	...
	sup_weekly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
												 string="Filter Supplier Weekly Balance Due")
	...
	def _compute_set_statments(self):
		...
		for rec in self:
			...
			if rec.statments == 'due':
				...
				sup_due_id = self.env['account.move'].search([('partner_id','=',rec.id),('move_type', 'in', ['in_invoice','in_refund','entry']),('state', 'in', ['posted']),('amount_residual_signed', '!=',0)])
				...
				sup_due_id = self.env['account.move'].search(
					[('partner_id','=',rec.id),('move_type', 'in', ['in_invoice', 'in_refund', 'entry']), ('state', 'in', ['posted'])])
				set = []
				for record in due_id:
		...
		for partner in self:
			...
			if partner.payment_amount_due_amt == 0.00:
				...
				if partner.email:
					...
					template = self.env.ref('bi_customer_overdue_statement.bi_email_template_customer_due_statement')
		...
		for partner in self:
			...
			if partners_to_email:
				...
				if partner not in partners_to_email:
					self.message_post([partner.id], body=_('Supplier Statement email sent to %s' % ', '.join(['%s <%s>' % (partner.name, partner.email) for partner in partners_to_email])))
