...
class SupplierPortal(CustomerPortal):
    ...
    def download_customer_report(self, access_token=None):
        partner = request.env.user.partner_id
        pdf = \
        request.env["ir.actions.report"].sudo()._render_qweb_pdf('bi_customer_overdue_statement.report_customer', partner.id)[
            0]
    ...
    def download_supplier_report(self, access_token=None):
        partner = request.env.user.partner_id
        pdf = request.env["ir.actions.report"].sudo()._render_qweb_pdf('bi_customer_overdue_statement.report_supplier',
                                                                partner.id)[0]
