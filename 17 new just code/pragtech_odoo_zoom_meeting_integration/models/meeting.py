...
class CustomZoomMeet(models.Model):
    ...
    def send_mail_notification_mail(self):
        self.env['calendar.attendee']._send_invitation_emails()

        # print("---------------testing")
        # try:
        #     _logger.info(_("Inside Send Mail Function : \t"))
        #     company_id = self.env['res.users'].search([('id', '=', self._context.get('uid'))]).company_id
        #     template_id = self.env['ir.model.data']._xmlid_lookup('pragtech_odoo_zoom_meeting_integration.send_mail_meeting_invitation_template_id_one_two_three')[1]
        ...
        #                 values = email_template_obj._generate_template([i], fields=None)
    ...
    def send_mail_notification_mail_user(self):
        ...
        # print("-------------------test mail user---------------------------")
        # try:
        #     print("-------------------test mail user inside try---------------------------")
        #     company_id = self.env['res.users'].search([('id', '=', self._context.get('uid'))])
        #     # print("\n\ncompany_id\t\t", company_id.outgoing_server_mail_id, "\n\n")
        #     template_id = self.env['ir.model.data']._xmlid_lookup('pragtech_odoo_zoom_meeting_integration.send_mail_meeting_invitation_user_template_id')[1]
        ...
        #                 values = email_template_obj._generate_template([i], ['subject', 'body_html', 'email_from', 'email_to',
