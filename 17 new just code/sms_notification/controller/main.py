...
class SMSNotificationDashboard(http.Controller):
    ...
    def load_dashboard(self,line_stage='all', pie_stage='all', days=7,**kw):
        ...
        for cnt_gateway in connected_gateways:
            reports = request.env['sms.report'].search([('sms_gateway_config_id.description','=',cnt_gateway.description)])
            web_base_url = request.env['sms.mail.server'].get_base_url()
            gateway_path  = Path('/sms_notification/static/sms_gateways/'+cnt_gateway.gateway+'.png')
            if gateway_path.exists():
               gateway_image = '/sms_notification/static/sms_gateways/'+cnt_gateway.gateway+'.png'
            else:
                gateway_image = '/sms_notification/static/description/icon.png'
