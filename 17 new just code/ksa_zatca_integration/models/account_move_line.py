...
class AccountMoveLine(models.Model):
    ...
    def onchange_tax_ids(self):
        if self.is_zatca:
            ...
            if len(list(set(self.move_id.invoice_line_ids.tax_ids.filtered(lambda x: x.classified_tax_category == 'E').mapped('tax_exemption_selection')))) > 1 or \
                    len(list(set(self.move_id.invoice_line_ids.tax_ids.filtered(lambda x: x.classified_tax_category == 'Z').mapped('tax_exemption_selection')))) > 1:
