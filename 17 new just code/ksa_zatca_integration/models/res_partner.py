...
class ResPartner(models.Model):
    ...
    buyer_identification_no = fields.Char(string="Buyer Identification Number (Other buyer ID)",
                                          help="|) required only if buyer is not VAT registered."
                                               ...
                                               "fill the commercial registration of the branch in respect of which "
                                               "the Tax Invoice is being issued.")
    l10n_sa_is_industry_id = fields.Boolean(compute="_compute_l10n_sa_is_industry_id")
    ...
    def _compute_l10n_sa_is_industry_id(self):
        for record in self:
            record.l10n_sa_is_industry_id = False
            company_ids = self.env["res.company"].sudo().search([])
            if record.id in company_ids.partner_id.ids:
                record.l10n_sa_is_industry_id = True
