...
class website_voucher(WebsiteSale):
	...
	def shop_payment_validate(self, transaction_id=None, sale_order_id=None, **post):
		response = super(website_voucher, self).shop_payment_validate(transaction_id=transaction_id, sale_order_id=sale_order_id, **post)
		if sale_order_id is None:
			sale_order_id = request.session.get('sale_last_order_id')
		order = request.env['sale.order'].sudo().browse(sale_order_id)
		product_id = request.env['ir.default'].sudo()._get('res.config.settings', 'wk_coupon_product_id')
