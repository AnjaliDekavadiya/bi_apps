...
class sale_order(models.Model):
	...
	def _compute_seller_amount(self):
		voucher_product_id = self.env['ir.default'].sudo()._get('res.config.settings', 'wk_coupon_product_id')
		coupon = self.order_line.filtered(lambda l: l.product_id.id == voucher_product_id)
		res = super(sale_order,self)._compute_seller_amount()
		if coupon and self.invoice_status == 'invoiced':
			self.seller_amount = self.seller_amount + coupon.price_subtotal
