...
class clouds_client(models.Model):
    ...
    dropbox_app_key = fields.Char(string="Dropbox app key")
    dropbox_app_secret = fields.Char(string="Dropbox app secret")
    dropbox_redirect_url = fields.Char(
        string="Dropbox redirect URL",
        default=_default_dropbox_redirect_url,
        help="The same redirect URL should be applied to your Dropbox app settings",
    )
    dropbox_team = fields.Boolean(
        string="Dropbox team",
        help="Turn on, if the sync should be established for the Dropbox team account",
    )
    dropbox_no_links = fields.Boolean(
        string="No links to Dropbox from Odoo",
        help="""If checked, you will not be able to open a Dropbox file or Dropbox folder from Odoo.
There might be 2 reasons to turn this setting on:
1. Personal Dropbox (Free, Plus, Professional plans) makes public links for all Odoo-related folders and files. It is\
not fully safe. Although website URLs will be hardly known by external users, there is a slight chance that they will\
become available as a result of some user actions
2. You would like to speed the sync up. During both direct and backward sync, Odoo tries to retrieve updated URLs of\
each Dropbox Odoo-related item. It results in a significant number of requests, which might make the process slow.""",)
    dropbox_admin = fields.Char(
        string="Dropbox admin email",
        help="Under this user all files and folders will be created, changed, and deleted in Dropbox",
    )
    dropbox_refresh_token = fields.Char(string="Dropbox active token", readonly=True, default="")
    ...
    def action_dropbox_establish_connection(self):
        ...
        if auth_url:
            ...
            res = {"name": "Dropbox", "type": "ir.actions.act_url", "url": auth_url}
        else:
    ...
    def _drb_get_auth_url(self):
        ...
        try:
            ...
            log_message = "The app confirmation URL was received"
        except Exception as e:
            auth_url = False
            log_message = "It is not possible to start the app confirmation. Reason: {}".format(e)
    ...
    def _drb_finish_auth(self, query_params):
        ...
        try:
            ...
            self._cloud_log(True, "Finishing the app confirmation", "DEBUG")
        ...
        if not this_client_ctx.get("cclients"):
            log_message = "It is not possible to finish the authentication since the client could not be initiated"
            self._cloud_log(False, log_message)
            return False, log_message
        ...
        if root_dir:
            ...
            log_message = "The app confirmation was finished"
            self._cloud_log(True, log_message)
            return True, log_message
        else:
            log_message = "The app confirmation was not finished. The root folder could not be created. Check the logs!"
            self._cloud_log(False, log_message)
            return False, log_message
    ...
    def _dropbox_get_client(self):
        ...
        try:
            if self.dropbox_team:
                ...
                if admin_user:
                    ...
                    log_message = "The team client was authenticated"
                else:
                    res_client = False
                ...
                log_message = "The client was authenticated"
        except Exception as er:
            res_client = False
            log_message = "The client was not initiated. Reason: {}".format(er)
        ...
        try:
            ...
            self._cloud_log(False, "Could not turn off the Dropbox logging: {}".format(er), "WARNING")
    ...
    def _dropbox_setup_sync(self, folder_id, attachment_id, cloud_key, args):
        ...
        result = {"cloud_key": result.get("id"), "url": result.get("url")}
        return result
    ...
    def _dropbox_change_attachment(self, folder_id, attachment_id, cloud_key, args):
        ...
        client = self._context.get("cclients", {}).get(self.id)
        cdata = self._dropbox_wrap_metadata(client.files_get_metadata(cloud_key))
        result = {"name": cdata.get("name"), "url": cdata.get("url")}
        return result
