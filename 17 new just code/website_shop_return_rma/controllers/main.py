...
#         'page_name': 'return',
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        partner = request.env.user.partner_id
        values['return_count'] = request.env['return.order'].sudo().search_count([
        ('partner_id', 'child_of', [partner.commercial_partner_id.id])
          ])
        return values
...
#                        'company' : request.env.user.company_id.name, odoo13
    ...
    @http.route(['/my/returns/printrma/custom<int:return_id>'], type='http', auth="public", website=True)
    def portal_rma_order_report(self, return_id, access_token=None, **kw):
        ...
        # pdf = request.env.ref('website_shop_return_rma.print_return_report').sudo()._render_qweb_pdf([return_id])[0]
        pdf = request.env['ir.actions.report'].sudo()._render_qweb_pdf('website_shop_return_rma.print_return_report', [return_id])[0]
        pdfhttpheaders = [
