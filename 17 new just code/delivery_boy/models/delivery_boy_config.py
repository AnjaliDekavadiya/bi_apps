...
class DeliveryBoyConfig(models.Model):
    ...
    name = fields.Char('Title', default="Delivery Boy Configuration", required=True)
    ...
    db_reset_password = fields.Boolean(string='Enable Reset Password', help="This allows users to trigger a password reset from App")
    show_banner = fields.Boolean(string='Show Banner on App', help="Allow this to show the delivery boy banner in App")
    verify_token = fields.Boolean(string='Enable Customer Token Verification',help="Before delivery confirmation of any picking delivery token will be verified")
    db_banner = fields.Binary('Banner')
    authentication_type = fields.Selection([
        ('user', 'User'),
        ('employee', 'Employee')],
        string='Authentication object', required=True,
        default='user',
        help="Check delivery boy program was enable for user or employee.")
    employee_ids=fields.Many2many('hr.employee',string="Employee Details",domain="[('is_delivery_boy','=',True)]")
    enable_parcle_photo=fields.Boolean(string="Enable Upload Parcel Image ",help="This feature allows to take the parcle image after delivery to ensure that product is delivered successfully.")
    ...
    def change_authentication_object(self):
        # for user in self:
        if self.authentication_type == "user":
            user_obj=self.env['res.users'].search([('is_delivery_boy','=',True)])
            if user_obj:
                return {
                    'name': "Warning",
                    'type': 'ir.actions.act_window',
                    'res_model': 'authentication.warning.wizard',
                    'view_mode': 'form',
                    'target': 'new',
                    }
            else:
                self.authentication_type = "employee"
        else:
            employee_obj=self.env['hr.employee'].search([('is_delivery_boy','=',True)])
            if employee_obj:
                return {
                    'name': "Warning",
                    'type': 'ir.actions.act_window',
    ...
    def fetch_user_info(self, user_obj, context=None):
        context = context or {}
        temp_i = {
        'deliveryBoyProfileImage': self._get_image_url('res.partner',user_obj.work_contact_id.id or user_obj.partner_id.id,'image_1920',user_obj.work_contact_id.write_date.__str__() or user_obj.partner_id.write_date.__str__(), context=context),
        'deliveryBoyName': user_obj.work_contact_id.name or user_obj.partner_id.name or user_obj.id.name or "",
        'deliveryBoyEmail': user_obj.work_email or user_obj.login or "",
        'deliveryBoyPhone': user_obj.work_phone or user_obj.phone or "",
        }
        return temp_i
    ...
    def authenticate(self, credentials, detailed = False, isSocialLogin=False, context=None):
        context = context or {}
        response = {'success':False, 'responseCode':400, 'message':_('Unknown Error !!!'), 'logout':False}
        user = False
        DeliveryBoyConfig_obj=self.search([],limit=1)
        ...
        elif DeliveryBoyConfig_obj.authentication_type == 'employee':
            if not all(k in credentials for k in ('login','pwd')):
                response['message'] = _('Insufficient data to authenticate !!!')
                return response
            try:
                user = self.env['hr.employee'].sudo().search([('barcode', '=', credentials['login']),('pin','=',credentials['pwd']),('work_contact_id.is_delivery_boy','=',True)])
                if not user:
                    response['responseCode'] = 400
                    response['logout'] = True
                    response['message'] = _("Invalid employee BadgeID/PIN address.")
            except:
                user = False
                response['responseCode'] = 400
        ...
        if user:
            try:
                response['success'] = True
                response['responseCode'] = 200
                response['deliveryBoyPartnerId'] =  user.work_contact_id.id or user.partner_id.id
                response['status'] = user.work_contact_id.delivery_boy_status or user.partner_id.delivery_boy_status
    ...
    def _getPickingDetails(self, db_picking):
        ...
        'orderType':db_picking.sale_order_id.order_type,
        # 'hasCOD': False,
        'products': [],
    ...
    def _formattedValue(self, value, symbol, position='after'):
        ...
        return "%s%s"%(format(value,'.2f'), symbol)
...
class DeliveryBoyNotificationMessages(models.Model):
    ...
    def create(self, vals):
        for val in vals:
            val['name'] = self.env['ir.sequence'].next_by_code('delivery.boy.notification.messages')
...
class DeliveryBoyFcmRegisteredDevices(models.Model):
    ...
    def _compute_display_name(self):
        res = []
        for record in self:
            name = record.partner_id and record.partner_id.display_name or ''
