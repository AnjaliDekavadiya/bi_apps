...
class DeliveryBoyPickings(models.Model):
    ...
    msg = fields.Text()
    check_order_type=fields.Char(compute="_check_order_type",store=True)
    cash_collected=fields.Boolean(string='Cash Collected')
    parcel_attachment = fields.Boolean(default=False) 
   
    @api.depends('sale_order_id')
    def _check_order_type(self):
        for rec in self:
            if rec.sale_order_id.order_type=='postpaid':
                rec.check_order_type='postpaid'
            else:
                rec.check_order_type='prepaid'
    ...
    def create(self, vals):
        for val in vals:
            val['name'] = self.env['ir.sequence'].next_by_code('delivery.boy.pickings')
            val['delivery_token'] = self.token(5)
    ...
    def stateDelivered(self,delivery_boy_config,delivery_boy_picking,response):
