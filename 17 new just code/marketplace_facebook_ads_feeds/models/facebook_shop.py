...
class FacebookShop(models.Model):
    ...
    def _get_default_domain(self):
        ...
        if self._context.get('mp_fb_feed'):
            domain.append(('marketplace_seller_id','in',[self.marketplace_seller_id.id]))
            return domain
    ...
    #     login_ids = []
    #     seller_group = self.env['ir.model.data']._xmlid_lookup(
    ...
    def test_function(self):
        if self._context.get('mp_fb_feed'):
            ...
            return {
                ...
                'view_mode':'tree,form',
                'res_model':'fb.attachment.mapping',
                'domain':[('id', 'in', mappings)],
                'views':[(self.env.ref('facebook_ads_feeds.fb_attachment_mapping_tree').id,'tree'),
                    (self.env.ref('marketplace_facebook_ads_feeds.fb_attachment_mapping_form_mp_facebook_ads_feeds').id,'form'),
                ],
            }
        action = super(FacebookShop,self).test_function()
        return action
