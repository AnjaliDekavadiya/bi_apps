...
class SaleOrderline(models.Model):
    ...
    def _delivery_state(self):
        for line in self:
            if line.move_ids:
                line.delivery_state = True if True in [proc.state in ['done', 'cancel'] for proc in line.move_ids] else False
            else:
    ...
    rma_ids = fields.One2many(
        string='rma',
        comodel_name='rma.rma',
        inverse_name='orderline_id',
    )
    ...
    def get_lot_ids(self):
        ...
        exist_sr_no = []
        for rma_id in self.rma_ids:
            exist_sr_no.extend(rma_id.lot_serial_ids.ids)

        return list(set(all_sr_no).difference(set(exist_sr_no)))    
    ...
    def rma_open_wizards_button(self):
        ...
        rma_refund_qty = 0
        order_line_qty = self.qty_delivered
        ...
        if rma_refund_qty >= order_line_qty:
            ...
            vals = {
                ...
                'problem': '',
                'tracking_type': product_id_line.tracking,
            }

            partial_id = self.env['rma.wizard'].create(vals)
            return {'name': "Return Merchandise Authorization",
                    'view_mode': 'form',
                            'view_id': False,
                            'view_type': 'form',
                            ...
                            'nodestroy': True,
                            'target': 'new',
                            'context': {'delivered_ids':self.get_lot_ids()}
                    }   
...
class SaleOrder(models.Model):
    ...
    def validate_picking_for_rma(self):
