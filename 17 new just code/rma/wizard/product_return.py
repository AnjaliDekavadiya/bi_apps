...
class ProductReturnWizard(models.TransientModel):
    ...
    def onchange_picking_type_id(self):
        ...
        if self.rma_id and self.rma_id.return_request_type == 'repair':
            repair_location_id = self.env['ir.default'].sudo()._get('res.config.settings', 'repair_location_id')
    ...
    @api.model
    def _get_lot_serial_ids(self):
        result = self.env['rma.rma'].browse(self._context['active_id'])
        return result.lot_serial_ids.ids

    def apply(self):
        self.ensure_one()
        # Create new picking for returned products
        serial_number = self._get_lot_serial_ids()
        ...
        x = self.rma_id.order_id
        order_line_id = x.order_line.filtered(lambda line: line.id==self.rma_id.orderline_id.id)
        order_line_id.qty_delivered = order_line_id.qty_delivered - self.rma_id.refund_qty if self.rma_id.refund_qty<=order_line_id.product_uom_qty else order_line_id.qty_delivered
        order_line_id.qty_to_invoice = -self.rma_id.refund_qty 
        return new_picking, pick_type_id 


class inheritProductInvoice(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        context.update({'active_id': order_id.id,'active_ids':[order_id.id]})
        self.env.context = context
        return super(inheritProductInvoice,self).create_invoices()
