...
$(document).ready(function() {
	...
	if(location.href.indexOf('attachment_id')!=-1){
		...
		$('.rma-return').on('click',function(e)
		{
			...
			jsonrpc("/return/rma/", {'order_line': order_line})
			.then(function (vals)
			{
	            var $modal = $(vals);
	            $modal.appendTo('#wrap')
		...
		$(document).on('click','#return_submit', function (event)
		{
			...
			var serial_number = $('#rma_serial_number').val();
			...
			if (!event.isDefaultPrevented()) {
				...
				else if (tracking == 'serial' && serial_number.length == 0)
				{
					$("#error-msg-serial").show();
					$("#resolution-div").addClass("has-error");
				}
				...
				else if (tracking == 'serial' && serial_number.length != parseInt(return_qty))
				{
					$("#error-equal-qty-ser").show();
					$("#resolution-div").addClass("has-error");
				}
				else
				{
		...
		{
			var order_line = parseInt($(this).find('.order_line').first().val(),10);
			jsonrpc("/rma/view/", {'order_line': order_line})
			.then(function (vals)
			{
	            var $modal = $(vals);
	            $modal.appendTo('#wrap')
		...
		$(document).on('change', "input#rma_product_qty", function()
		{
			...
			{
				let warning = "You can return only " +max_return_qty + " quantity."
				swal("Invalid Quantity", warning, "warning");
				$('#rma_product_qty').val(max_return_qty);
				$('#refund_amount_span .oe_currency_value').text((parseFloat($('#rma_product_qty').val()) * unit_price).toString());
				$('#rma_refund_amount').val((parseFloat($('#rma_product_qty').val()) * unit_price).toString());
			}
