...
class VehicleContract(models.Model):
    ...
    rent_type = fields.Selection([('days', "Days"), ('week', "Weeks"), ('month', "Months"),
                                  ('km', "Kilometers"), ('mi', 'Miles')], string="Rent Type")
    ...
    journal_id = fields.Many2one('account.journal', domain=[('type', 'in', ('bank', 'cash'))], string="Deposit Journal")
    date = fields.Date(string="Date")
    signature = fields.Binary(string="Signature")

    payment_type = fields.Selection([('full_payment', "Full Payment"), ('monthly', "Monthly"),
                                     ('weekly', "Weekly"), ('quarterly', "Quarterly")], string="Payment Type")
        ...
        for record in self:
            if not record.rent_type:
                raise ValidationError(
                    _("Choose your preferred rental unit (days, week, month, kilometers, or miles) and proceed accordingly."))
        ...
        for rec in self:
            ...
            if rec.end_date and rec.start_date and not (rec.start_date > rec.end_date):
                if rec.rent_type == 'days':
                    ...
                    count = days_diff.days
                elif rec.rent_type == 'week':
                    days_diff = rec.end_date - rec.start_date
                    total_days = days_diff.days
                    weeks_between = total_days / 7
                    count = weeks_between
                elif rec.rent_type == 'month':
                    months_difference = (rec.end_date.year - rec.start_date.year) * 12 + (
                            rec.end_date.month - rec.start_date.month)
                    start_date_day = rec.start_date.day
                    end_date_day = rec.end_date.day
                    remaining_days = (end_date_day - start_date_day) / \
                                     calendar.monthrange(rec.start_date.year, rec.start_date.month)[1]
                    count = months_difference + remaining_days
            rec.total_days = count
    ...
    def _get_total_vehicle_rent(self):
        for rec in self:
            ...
            if rec.rent and rec.rent_type:
                if rec.rent_type == 'days' or rec.rent_type == 'week' or rec.rent_type == 'month':
