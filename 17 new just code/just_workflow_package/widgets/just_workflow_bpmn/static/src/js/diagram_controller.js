...
export class DiagramPlusController extends Component {
	...
	static components = { Layout,CogMenu,useSearchBarToggler,SearchBar};

	setup() {
		this.rpc = jsonrpc;
        this.displayDialog = useOwnedDialogs();
		this.className = 'o_diagram_plus_view';
	...
	async saveDiagram(params = {}) {
		var Modeling = window.$diagram.get("modeling");
	...
	async _downbpmn(e) {
		window.$diagram.saveXML({ format: true }, (err, xml) => {
			this.download('bpmn', xml);
		});
	};

	async _downsvg(e) {
		window.$diagram.saveSVG({ format: true }, (err, xml) => {
			this.download('svg', xml);
		});
	};

	async _importbpmn(e) {
		if (confirm(_t('import will remove all of the workflow diagram, please confirm!')) == true) $('#fileSelector').click();
	};
	...
	processFileSelected(e) {
		...
		function importSuccessCallback(xml) {
			window.$diagram.importXML(xml, err => {
				if (err) {
	...
	async _flowsave(e) {
		var self = this;
		var elements = window.$diagram.get("elementRegistry")._elements;
		var Modeling = window.$diagram.get("modeling");
	...
	async _flowDbSave(e) {
		...
		var diagram = await window.$diagram.saveXML({ format: true });
				...
				if (self.mode === 'view') {
					...
					$.map(elements, function (element) {
						...
						if (element.type.indexOf('Flow') > 0 && self.props.flowstate.transIds.includes(bpmnId)) {
							...
							var indexName = (indexOfTrans + 1) + '-' + self.props.flowstate.logs[indexOfTrans].trans_id[1];
