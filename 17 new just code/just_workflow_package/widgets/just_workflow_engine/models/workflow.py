...
class workflow_base(models.Model):
    ...
    def _default_reset_group(self):
        return self.env['ir.model.data']._xmlid_to_res_id('base.group_system')
  ...
  <bpmndi:BPMNDiagram id="BpmnDiagram_1">
    ...
    def make_btm_contain(self):
        ...
        for t in self.trans_ids:
            ...
            btn = XML(btn_template % {'btn_str': _(btnName) + t.name, 'trans_id': t.id, 'vis_state': t.node_from.id,
...
for record in self:
    ...
    def event_model(self):
        ...
        for rec in records:
            event_model_list.extend([ i.task_model for i in rec.trans_ids if i.task_model ])
...
class workflow_node(models.Model):
    ...
    show_state = fields.Boolean('Show In States', default=False,
    ...
    event_need = fields.Boolean('Create event', default=True,
    ...
    def make_event(self, name, ids, form_record):
        ...
        for id in ids:
            self.env['mail.activity'].create({
                ...
                'res_model_id': self.env['ir.model']._get(form_record._name).id,
...
class workflow_trans(models.Model):
    ...
    workflow_id = fields.Many2one('workflow.base', related='node_from.workflow_id', store=True)
    ...
    def make_log(self, res_name, res_id, note=''):
        return self.env['log.workflow.trans'].create({'name': res_name, 'res_id': res_id, 'trans_id': self.id, 'node_id': self.node_from.id, 'note': note, 'username': self.env.user.name})
...
class RunCode:
    ...
    def _run_code(self):
        ...
        locals_dict = {
            'env': self.env,
            'model': self.env[cx.get('workflow_model_name', False)],
            'record': self.env[cx.get('workflow_model_name', False)].browse(cx.get('workflow_active_id', 0)),
            'records': self.env[cx.get('workflow_model_name', False)].browse(cx.get('workflow_active_ids', 0)),
            'user': self.env.user,
            'datetime': datetime,
            'time': time,
            'date': date,
        ...
        for rec in self.Model:
            try:
                eval(rec.py_code, locals_dict=locals_dict, mode='exec', nocopy=True)
