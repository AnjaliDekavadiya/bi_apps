...
function makeActionManager(env) {
    ...
    let id = 0;
    // remove this stackm all the information stored in the controller sub stack
    // let controllerStack = [];
    let dialogCloseProm;
    let actionCache = {};
    let dialog = null;
    ...
    function _getControllerStack(actionJsId) {
        if (actionJsId) {
            return aklStacks[actionJsId] || (aklStacks[actionJsId] = []);
        } 
        return null
    }
    ...
    function _getActionMode(action) {
        ...
        const controllerStack = _getControllerStack(action.jsId);
    ...
    async function _updateUI(controller, options = {}) {
            ...
            onMounted() {
                if (action.target === "new") {
                    ...
                    if (action.ref_action) {
                        aklStacks[action.ref_action.jsId] = nextStack;
                    } else {
                        aklStacks[action.jsId] = nextStack;
                    }
                    // currentActionJsId = action.jsId;
                    pushState(controller);
                    if (action.jsId === currentActionJsId) {
        ...
        // add refaction
        if (index != 0) {
            controller.action.ref_action = aklStacks[oldActionJsId][0].action
        }
