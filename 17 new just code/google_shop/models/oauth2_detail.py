...
class ShopAuth(models.Model):
    ...
    def _default_configuration_calculate(self):
        for record in self:
            email = self.env['res.config.settings'].get_values()[
                'admin_email']
            if not email:
                _logger.info(_("Please insert notification email address in Google Config settings"))
            record.email = email
    ...
    @api.onchange('domain_url', 'sequence_no')
    def _compute_callback(self):
        for domain_sequence in self:
            if domain_sequence.domain_url and domain_sequence.sequence_no:
                domain_sequence.callback_url = domain_sequence.domain_url + \
