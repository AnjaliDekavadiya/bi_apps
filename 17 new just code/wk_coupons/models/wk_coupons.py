...
class VoucherVoucher(models.Model):
	...
	def get_default_values(self, field=False):
		...
		if field:
			if field == 'wk_coupon_customer_type' and self._context.get('is_template'):
				return False
			return irDefault._get('res.config.settings', '%s'%field)
		vals['product_id'] =  irDefault._get('res.config.settings', 'wk_coupon_product_id')
		vals['default_validity'] =  irDefault._get('res.config.settings', 'wk_coupon_validity') or -1
		vals['max_amount'] =  irDefault._get('res.config.settings', 'wk_coupon_max_amount') or 1000
		vals['min_amount'] =  irDefault._get('res.config.settings', 'wk_coupon_min_amount') or 0
		vals['max_expiry_date'] =  irDefault._get('res.config.settings', 'wk_coupon_max_expiry_date') or datetime.now().date()
		vals['default_availability'] =  irDefault._get('res.config.settings', 'wk_coupon_validity') or 1
		vals['default_value'] =  irDefault._get('res.config.settings', 'wk_coupon_value') or 100
		vals['partially_use'] =  irDefault._get('res.config.settings', 'wk_coupon_partially_use') or False
		vals['voucher_usage'] =  irDefault._get('res.config.settings', 'wk_coupon_voucher_usage') or 'both'
		vals['customer_type'] =  irDefault._get('res.config.settings', 'wk_coupon_customer_type') or 'general'
		vals['partial_limit'] =  irDefault._get('res.config.settings', 'wk_coupon_partial_limit') or -1
		vals['use_minumum_cart_value'] =  irDefault._get('res.config.settings', 'wk_coupon_minumum_cart_value_usage') or False
		vals['minimum_cart_amount'] =  irDefault._get('res.config.settings', 'wk_coupon_minimum_cart_amount') or 1000
		vals['wk_coupon_name'] =  irDefault._get('res.config.settings', 'wk_coupon_name') or 'Default Coupon Name'
		return vals
	...
	def send_mail_to_customers(self):
		if self.customer_type == 'special_customer':
			...
			if res:
				wizard_id = self.env['wk.wizard.message'].create({'text':'Email has been sent successfully to the selected customer..'}).id
	...
	def create(self, vals):
		...
		if vals['voucher_value'] < self.get_default_values('wk_coupon_min_amount') and vals['voucher_val_type'] != 'percent':
			raise  ValidationError(('You can`t create voucher below this minimum amount (%s) !!!')%self.get_default_values('wk_coupon_min_amount'))
		if vals['voucher_value'] > self.get_default_values('wk_coupon_max_amount') and vals['voucher_value_type'] != 'percent':
	...
	def write(self, vals):
		for voucher in self:
			...
			if voucher_value < self.get_default_values('wk_coupon_min_amount') and voucher_value_type != 'percent':
				raise ValidationError(('You can`t create voucher below this minimum amount (%s) !!!')%self.get_default_values('wk_coupon_min_amount'))
			if vals.get('percnet'):
				if vals.get('voucher_value') < 0 or vals.get('voucher_value') > 100:
					raise  ValidationError(('The percentage value should be within 0 and 100'))
