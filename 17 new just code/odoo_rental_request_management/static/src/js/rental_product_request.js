...
publicWidget.registry.RentalProductSale = publicWidget.Widget.extend({
        ...
        start: function(){
            ...
            $.each($('.input-group.date'), function(date_field){
                // $(date_field).data('mindate') || Math.floor({ y: today_date.getFullYear(), m:today_date.getMonth(), d:today_date.getDate() });
                var minDate = $(date_field).data('mindate') || ({ y: today_date.getFullYear(), m:today_date.getMonth(), d:today_date.getDate() });
                ...
                //     },
                //     locale : serializeDateTime(DateTime.local()),
        ...
        _ResetSaleorderLine: function(args){
            ...
            $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: '/json/reset/rental/soline',
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify({'jsonrpc': "2.0", 'method': "call", "params": args}),
        ...
        _prepare_json_rental_product_avail_price_vals: function(ev){
            ...
            var selectedOption = $('select[name="rental_product_variant_select"] option:selected');
            var price_float = selectedOption.data('lst-price');
        ...
        _CheckProductAvailability: function(ev){
            ...
            var qty = $(".rental_product_fade_qty").val()
            var days = $(".rental_product_fade_days").val()
            var json_check_avail_vals = this._prepare_json_rental_product_avail_price_vals()
            ...
            $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: '/json/rental/product/avail/status',
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify({'jsonrpc': "2.0", 'method': "call", "params": json_check_avail_vals}),
                    success: function (data) {
                        if (data.result.reserved == true){
                            $(".o_js_rental_avail_status").addClass('alert-warning')
                            $(".o_js_rental_avail_status").removeClass('alert-success')
                            $("#rental_product_avail_status").html(data.result.availability_msg)
                            $(".buy_rental_product_fade").attr("disabled", true);
                        }
                        else{
                            $(".o_js_rental_avail_status").addClass('alert-success')
                            $(".o_js_rental_avail_status").removeClass('alert-warning')
                            $("#rental_product_avail_status").html(data.result.availability_msg)
                            if (data.result.first_load_fade == true){
                                $(".buy_rental_product_fade").attr("disabled", true);
                            }
                            else{
                                $(".buy_rental_product_fade").attr("disabled", false);
        ...
        _ChangeRentalPrice: function(ev){
            ...
            $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: '/json/rental/product/price',
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify({'jsonrpc': "2.0", 'method': "call", "params": json_so_vals}),
                    success: function (data) {
                        self.$variantPrice = data.result.rental_price;
                        // var rental_price = self._price_to_str(parseFloat(self.$variantPrice))
                        var rental_price = self._priceToStr(parseFloat(data.rental_price))
                        self.$CurrencySymbol
                        self.$CurrencySymPosition
                        
                        if (self.$CurrencySymPosition == 'before'){
                            rental_price = self.$CurrencySymbol + ' ' + data.result.rental_price;
                        }
                        else{
                            rental_price = data.result.rental_price; + ' ' + self.$CurrencySymbol
                        }
                        $("span.rental_variant_price").html(rental_price)
                        $(".rental_product_fade_price").val(parseFloat(self.$variantPrice))
                    }
