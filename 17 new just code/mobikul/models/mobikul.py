    ...
    def authenticate(self, credentials, detailed=False, isSocialLogin=False, context=None):
        ...
        if user:
            ...
            try:
                ...
                response['cartCount'] = user.partner_id.sudo().last_mobikul_so_id and user.partner_id.sudo().last_mobikul_so_id.cart_count or 0
                response['message'] = _('Login successfully.')
                context.update({"partner": user.partner_id, "uid": user.id,
                                "user": user, 'tz':context.get('tz') or user.tz})
                ...
                if self.check_mobikul_addons().get('odoo_marketplace'):
                    response['is_seller_requested'] = True if user.partner_id.seller else False
