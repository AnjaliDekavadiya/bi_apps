...
from odoo.addons.mobikul.tool.help import _displayWithCurrency, _get_image_url, _get_ar_url,_changePricelist, remove_htmltags, AQUIRER_REF_CODES, STATUS_MAPPING, _get_next_reference,EMPTY_ADDRESS, _easy_date,_get_product_domain
    ...
    def getMyOrders(self, **kwargs):
        ...
        if response.get('success'):
            ...
            if Partner:
                ...
                for order in orders:
                    order_date = request.env['ir.qweb.field.datetime'].sudo().value_to_html(order.date_order,{})
        ...
        if response.get('success'):
            ...
            if Partner:
                if request.httprequest.method == "POST":
                    ...
                    OrderLine = OrderLineObj.sudo().search([("id","=",line_id)])
...
#
    ...
    @route(['/mobikul/contactUs'],csrf=False,type='http',auth="public",methods=['GET'])
