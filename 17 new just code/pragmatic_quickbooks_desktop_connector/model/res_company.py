...
class Qb_Config_Settings(models.Model):
    ...
    def check_duplicates(self):
        ...
        self.env.cr.execute("SELECT id,name,quickbooks_id FROM res_partner WHERE supplier_rank= 0 AND name IN (SELECT name FROM res_partner WHERE supplier_rank= 0 GROUP BY name HAVING (COUNT(name) > 1))")
# (ADDED above THIS QUERY (RANI))
        result = self.env.cr.fetchall()
        # print("\n\n\nDuplicate",result)
        if result:
            for row in result:
    ...
    def import_product(self):
        ...
        if not self.last_imported_qbd_id_for_product:
            ...
            try:
                ...
                raise UserError(e)
            formatted_data = json.loads(response.text)
            # print("responsee @@@@@@@@@@@@@@@@@@", formatted_data)
