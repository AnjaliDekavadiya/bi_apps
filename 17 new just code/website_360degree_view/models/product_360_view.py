...
class ProductTemplate(models.Model):
    ...
    def check_number_of_images_for_360(self):
        if self.product_360_view and self.number_of_images_for_360 == 0:
            raise UserError(_('Please add at least one image for product 360° view.'))


    @api.onchange('product_360_view')
    def on_change_product_360_view(self):
        self.product_default_view = not self.product_360_view

    @api.onchange('product_default_view')
    def on_change_product_default_view(self):
        self.product_360_view = not self.product_default_view
