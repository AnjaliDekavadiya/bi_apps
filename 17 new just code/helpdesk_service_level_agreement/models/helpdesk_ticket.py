...
class HelpdeskSupport(models.Model):
    ...
    def _onchange_partner_id(self):
        for rec in self:
            rec.level_config_id = rec.partner_id.level_config_id or rec.partner_id.commercial_partner_id.level_config_id or False
...
#                     sla_level_id = self.env['helpdesk.level.config'].sudo().browse(level_config_id)
                    custom_request_sate = vals.get('request_date') or ticket.request_date
...
#                        dead_line_date = ticket._compute_deadline(ticket.request_date, sla_level_id, vals)
                    if custom_request_sate and sla_level_id:
                        dead_line_date = ticket._compute_deadline(custom_request_sate, sla_level_id, vals)
