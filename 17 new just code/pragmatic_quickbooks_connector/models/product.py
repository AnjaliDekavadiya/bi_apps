            ...
            if 'ParentRef' in category:
                ...
                if not categ_obj:
                    ...
                    if company.import_pc_by_date:
                        date_format = '%Y-%m-%d'
                        if company.prodct_catgry_import_by == 'crt_dt':
                            date_string = category.get('MetaData').get(
                                'CreateTime')[:10]
                        else:
                            date_string = category.get('MetaData').get(
                                'LastUpdatedTime')[:10]
                        ...
                        date_object = datetime.strptime(date_string,
                                                        date_format).date()
                        company.import_pc_date = date_object
                ...
                if product_id.qbo_product_id:
                    ...
                    if update_product:
                        ...
                        if synctoken:
                            ...
                            _logger.info(
                                'Parsed Dict for update: {}'.format(parsed_dict))
                    ...
                    parsed_dict = json.dumps(vals)
                    _logger.info(
                        'Parsed Dict for create: {}'.format(parsed_dict))
                    ...
                    if result.status_code == 200:
                        ...
                        if resp_parsed.get('Item').get('Id'):
                            ...
                            product_id.qbo_product_id = resp_parsed.get(
                                'Item').get('Id')
                            _logger.info(_("Product exported sucessfully! product template Id: %s" % (
                                product_id.qbo_product_id)))
                            self._cr.commit()
                    ...
                    if created:
                        ...
                        if company.import_product_by_date:
                            ...
                            if company.prodct_import_by == 'crt_dt':
                                date_string = product.get('MetaData').get(
                                    'CreateTime')[:10]
                            else:
                                date_string = product.get('MetaData').get(
                                    'LastUpdatedTime')[:10]
                            ...
                            date_object = datetime.strptime(date_string,
                                                            date_format).date()
                            company.import_product_date = date_object
                else:
    ...
    def export_product_to_qbo(self):
        ...
        for product_id in self:
            ...
            if not product_id.default_code:
                raise ValidationError(
                    'Please Set Internal Reference for Product: {}'.format(product_id.name))
