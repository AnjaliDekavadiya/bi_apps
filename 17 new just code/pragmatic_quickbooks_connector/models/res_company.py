...
class ResCompany(models.Model):
    ...
    def import_customer_vendor_cron(self):
        ...
        company.import_customers(call_from='cron')
        _logger.info("Customers imported successfully.")
        self._cr.commit()
        # For importing vendors
        company.import_vendors(call_from='cron')
        _logger.info("Vendors imported successfully.")
        self._cr.commit()
    ...
    account_import_by = fields.Selection(
        ...
        string='Import By', default="crt_dt")
    ...
    # @api.multi
    def import_customers(self, call_from=None):
        # try:
        company = self.env['res.users'].search(
            [('id', '=', self._uid)]).company_id
        if not company:
        ...
        if company.import_customer_by_date:
            if self.cutomer_import_by == 'crt_dt':
                query = "select * from Customer WHERE Metadata.CreateTime >= '%s' AND ID >= '%s' order by Id STARTPOSITION %s MAXRESULTS %s " % (
                    company.import_customer_date, company.last_imported_customer_id, company.start, company.limit)
            else:
                query = "select * from Customer WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s' order by Id STARTPOSITION %s MAXRESULTS %s " % (
                    company.import_customer_date, company.last_imported_customer_id, company.start, company.limit)
        ...
        if data.status_code == 200:
            ...
            if self.import_mapping_customer_field and self.env.context.get('mapping'):
                if res.get('QueryResponse', False) and res.get('QueryResponse').get('Customer', []):
                    self.import_mapping_customer_id.with_context(
    ...
    def import_vendors(self, call_from=None):
        # try:
        company = self.env['res.users'].search(
        ...
        try:
            ...
            if company.import_account_by_date:
                if self.account_import_by == 'crt_dt':
                    query = "select * from Account WHERE Metadata.CreateTime >= '%s' AND ID >= '%s'" % (
                        company.import_account_date, company.last_acc_imported_id)
                else:
                    query = "select * from Account WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s'" % (
                        company.import_account_date, company.last_acc_imported_id)
            else:
            ...
            for account_id in account_ids:
                ...
                if account_id.qbo_id:
                    res = requests.request(
                        'GET', url + "/account/{}?minorversion=12".format(account_id.qbo_id), headers=headers,
                        data=outdict)
                    synctoken = '0'
    ...
    def import_tax(self):
        try:
            ...
            if company.import_tax_by_date:
                if self.tax_import_by == 'crt_dt':
                    query = "select * from TaxCode WHERE Metadata.CreateTime >= '%s' AND ID >= '%s'" % (
                        company.import_tax_date, company.last_imported_tax_id)
                else:
                    query = "select * from TaxCode WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s'" % (
                        company.import_tax_date, company.last_imported_tax_id)
            else:
        ...
        try:
            ...
            if company.import_pc_by_date:
                if self.prodct_catgry_import_by == 'crt_dt':
                    query = "select * from Item WHERE Type='Category' and Metadata.CreateTime >= '%s' AND ID >= '%s'" % (
                        company.import_pc_date, company.last_imported_product_category_id)
                else:
                    query = "select * from Item WHERE Type='Category' and Metadata.LastUpdatedTime >= '%s' AND ID >= '%s'" % (
                        company.import_pc_date, company.last_imported_product_category_id)
            else:
                query = "select * from Item where Type='Category' and Id > '%s' order by Id" % (
    ...
    # @api.multi
    def import_product(self, call_from = None):
        try:
            #         self.ensure_one()
            company = self.env['res.users'].search(
                [('id', '=', self._uid)]).company_id
            ...
            if company.import_product_by_date:
                if company.prodct_import_by == 'crt_dt':
                    query = "select * from Item where Metadata.CreateTime > '%s' AND ID >= '%s' order by Id STARTPOSITION %s MAXRESULTS %s  " % (
                        company.import_product_date, company.last_imported_product_id, company.start, company.limit)
                else:
                    query = "select * from Item where Metadata.LastUpdatedTime > '%s' AND ID >= '%s' order by Id STARTPOSITION %s MAXRESULTS %s  " % (
                        company.import_product_date, company.last_imported_product_id, company.start, company.limit)
            else:
    ...
    def export_product_mapping(self):
        ...
        if self.export_mapping_product_field and self.export_mapping_product_id:
            ...
            for product_id in product_ids:
                ...
                headers['Content-Type'] = 'application/json'
                parsed_dict = json.dumps(outdict)
                result = requests.request('POST', url + "/item?operation=update&minorversion=12", headers=headers,
                                          data=parsed_dict)
                if result.status_code == 200:
        ...
        try:
            ...
            if company.import_payment_method_by_date:
                if self.paymnt_method_import_by == 'crt_dt':
                    query = "select * from PaymentMethod WHERE Metadata.CreateTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_payment_method_date, company.last_imported_payment_method_id)
                else:
                    query = "select * from PaymentMethod WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_payment_method_date, company.last_imported_payment_method_id)

            else:
                query = "select * From PaymentMethod WHERE Id > '%s' order by Id" % (
                    company.last_imported_payment_method_id)
        ...
        try:
            ...
            if company.import_cp_by_date:
                if company.customer_paymnt_import_by == 'crt_dt':
                    query = "select * from Payment WHERE MetaData.CreateTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_cp_date, company.last_imported_payment_id)
                elif company.customer_paymnt_import_by == 'other_dt':
                    query = "select * from Payment WHERE TxnDate >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_cp_date, company.last_imported_payment_id)
                else:
                    query = "select * from Payment WHERE MetaData.LastUpdatedTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_cp_date, company.last_imported_payment_id)
            else:
        ...
        try:
            ...
            if company.import_vp_by_date:
                if company.vendor_paymnt_import_by == 'crt_dt':
                    query = "select * from billpayment WHERE Metadata.CreateTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_vp_date, company.last_imported_bill_payment_id)
                elif company.vendor_paymnt_import_by == 'other_dt':
                    query = "select * from billpayment WHERE TxnDate >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_vp_date, company.last_imported_bill_payment_id)
                else:
                    query = "select * from billpayment WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_vp_date, company.last_imported_bill_payment_id)
            else:
                query = "select * From billpayment WHERE Id > '%s' order by Id" % (
            ...
            if company.import_payment_term_by_date:
                if company.paymnt_term_import_by == 'crt_dt':
                    data = requests.request('GET', company.url + str(
                        company.realm_id) + "/query?query=select * from term where Metadata.CreateTime >= '{}' AND ID >= '{}'".format(
                        str(company.import_payment_term_date), str(company.x_quickbooks_last_paymentterm_imported_id)),
                                            headers=headers)
                else:
                    data = requests.request('GET', company.url + str(
                        company.realm_id) + "/query?query=select * from term where Metadata.LastUpdatedTime >= '{}' AND ID >= '{}'".format(
                        str(company.import_payment_term_date), str(company.x_quickbooks_last_paymentterm_imported_id)),
                                            headers=headers)
            else:
                data = requests.request('GET', company.url + str(
            ...
            if company.access_token:
                ...
                if company.import_employee_by_date:
                    if self.employee_import_by == 'crt_dt':
                        query = "select * from employee WHERE Metadata.CreateTime > '%s' AND ID >= '%s' order by Id " % (
                            company.import_employee_date, company.quickbooks_last_employee_imported_id)
                    else:
                        query = "select * from employee WHERE Metadata.LastUpdatedTime > '%s' AND ID >= '%s' order by Id " % (
                            company.import_employee_date, company.quickbooks_last_employee_imported_id)
            ...
            if company.access_token:
                ...
                if company.import_department_by_date:
                    if self.department_import_by == 'crt_dt':
                        query = "select * from department WHERE Metadata.CreateTime > '%s' AND ID >= '%s' order by Id " % (
                            company.import_department_date, company.quickbooks_last_dept_imported_id)
                    else:
                        query = "select * from department WHERE Metadata.LastUpdatedTime > '%s' AND ID >= '%s' order by Id " % (
                            company.import_department_date, company.quickbooks_last_dept_imported_id)
                else:
                    query = "select * from department WHERE Id > '%s' order by Id" % (
            ...
            for department_id in department_ids:
                ...
                for fields_line_id in self.export_mapping_department_id.fields_lines:
                    ...
                    if len(split_key) > 1:
                        ...
                        outdict[split_key[0]] = values
                parsed_dict = json.dumps(outdict)
                result = requests.request('POST', url + "/department?minorversion=63", headers=headers,
                                          data=parsed_dict)
                if result.status_code == 200:
                    response = result.json()
                            ...
                            for cust in parsed_data.get('QueryResponse').get('Estimate'):
                                if "CustomerRef" in cust and cust.get('CustomerRef').get('value'):
                                    ...
                                    if not sale_order:
                                        ...
                                        if res_partner:
                                            ...
                                            for i in cust.get('Line'):
                                                if i.get(
                                                    ...
                                                    discount_per = i.get('DiscountLineDetail').get('DiscountPercent')
                                                    break
                                            for i in cust.get('Line'):
                                                _logger.info(
                                        ...
                                        res_partner = self.env['res.partner'].search(
                                            [('qbo_customer_id', '=', cust.get('CustomerRef').get('value'))], limit=1)
                                        _logger.info(
                                            ...
                                            for i in cust.get('Line'):
                                                ...
                                                if 'SalesItemLineDetail' in i and i.get('SalesItemLineDetail'):
                                                    ...
                                                    if not res_product and i.get('SalesItemLineDetail').get(
                                                        ...
                                                        self._cr.commit()
                                                        _logger.info(
                                                            "Sale order line --------------->{}".format(create_po))
                                                        if create_po:
                                                            company.quickbooks_last_sale_imported_id = int(
                                                                cust.get('Id'))
                                                    if shipping_line:
    ...
    def import_transfer(self):
        try:
            ...
            if data.status_code == 200:
                ...
                last_imported_id = self.env[
                    'account.payment'].create_transfer(data)
                if last_imported_id:
                    company.quickbooks_last_trns_imported_id = last_imported_id
                    success_form = self.env.ref(
                        'pragmatic_quickbooks_connector.import_successfull_view', False)
                    return {
                        'name': _('Notification'),
    ...
    def import_deposit(self):
        try:
            ...
            if data.status_code == 200:
                ...
                last_imported_id = self.env[
                    'account.payment'].create_deposit(data)
                if last_imported_id:
                    company.quickbooks_last_dp_imported_id = last_imported_id
                    success_form = self.env.ref(
                        'pragmatic_quickbooks_connector.import_successfull_view', False)
                    return {
                        'name': _('Notification'),
    ...
    #     creditmemo_obj = self.env['account.move']
    #     creditmemo_obj.import_vendor_credit()
    #     _logger.info("Credit Memo imported successfully.")
    #     self._cr.commit()
    #     success_form = self.env.ref(
    #         'pragmatic_quickbooks_connector.import_successfull_view', False)
    ...
    def import_expenses(self):
        try:
            ...
            if data.status_code == 200:
                ...
                last_imported_id = self.env[
                    'account.payment'].create_expenses(data)
                if last_imported_id:
                    company.quickbooks_last_expns_imported_id = last_imported_id
                    success_form = self.env.ref(
                        'pragmatic_quickbooks_connector.import_successfull_view', False)
                    return {
                        'name': _('Notification'),
                                ...
                                if "CustomerRef" in cust and cust.get('CustomerRef').get('value'):
                                    ...
                                    if not sale_receipt:
                                        ...
                                        if not res_partner:
                                            ...
                                            _logger.info(
                                                "Dictionary is--->{}:".format(dict_s))
                                            if cust.get("DepositToAccountRef") and cust.get("DepositToAccountRef").get(
                                                    "value"):
                                                ...
                                                if not account_1:
                                                    ...
                                                    if not journal_1:
                                                        raise ValidationError(
                                                            _(f'Configure Account QBO ID {cust.get("DepositToAccountRef").get("value")} In Journal For Payment'))
                                            else:
                                                raise ValidationError(
                                            ...
                                            for i in cust.get('Line'):
                                                ...
                                                if 'SalesItemLineDetail' in i and i.get('SalesItemLineDetail'):
                                                    ...
                                                    if not res_product and i.get('SalesItemLineDetail').get(
                                                        ...
                                                        self._cr.commit()
                                                        _logger.info(
                                                            "Sales Receipt line --------------->{}".format(create_p))
                                                        if create_p:
                                            ...
                                            payment.qbo_salesreceipt_id = cust.get("Id")
                                            self._cr.commit()
                                    ...
                                    #         _logger.info(
                                    #             "Dict for update is ----> {}".format(dict_s))
                                    #         update_so = sale_receipt.write(
                                    #             dict_s)
                                    #         _logger.info(
                                    #             "update obj {}".format(update_so))
                                    #         if update_so:
                                    #             _logger.info(
                                    #                 'Sales Receipt Updated...!!! :: %s', cust.get('Id'))
                                    #         custom_tax_id = None
                                    #         discount_per = 0
                                    #         # for i in cust.get('Line'):
                                    #         #     if i.get(
                                    ...
                                    #         #                         'tax_ids'] = custom_tax_id
                                    #         #             _logger.info(
                                    #         #                 "Dictionary for sales Receipt line is --------> {}".format(
                                    #         #                     dict_l))
                                    #         #
                                    #         #             create_po = self.env['account.move.line'].search(
                                    #         #                 ['&', ('product_id', '=', res_product.id),
                                    #         #                  (('move_id', '=', sale_receipt.id))], limit=1)
                                    #         #
                                    #         #             if create_po:
                                    #         #                 res = create_po.update(dict_l)
                                    #         #
                                    #         #             self._cr.commit()
                                    #         #             _logger.info(
                                    #         #                 "Sales Receipt line --------------->{}".format(create_po))
                                    #         #             if create_po:
                                    #         #                 company.quickbooks_last_sales_receipt_imported_id = int(
                                    #         #                     cust.get('Id'))
                                    #         #
                                    ...
                                    #         #                 create_po = self.env['account.move.line'].search(
                                    #         #                     ['&', ('product_id', '=', res_product.id),
                                    #         #                      (('order_id', '=', sale_receipt.id))], limit=1)
                                    #         #
                                    #         #                 if create_po:
                                    #         #                     res = create_po.update({
                                    #         #
                                    ...
                                    #         #
                                    #         #             else:
                                    #         #                 '''CODE FOR NEW LINE IN EXISTING sales Receipt'''
                                    #         #                 _logger.info(
                                    #         #                     "Code for new line in existing sales Receipt")
                                    #         #                 res_product = self.env['product.product'].search(
                                    #         #                     [('qbo_product_id', '=',
                                    ...
                                    #         #                     # dict_l['product_uom'] = 1
                                    #         #                     _logger.info(
                                    #         #                         "Sales Receipt line of update is ----> {}".format(
                                    #         #                             dict_l))
                                    #         #                     create_p = self.env[
                                    #         #                         'account.move.line'].create(dict_l)
                                    #         #                     _logger.info(
                                    #         #                         "Sale Receipt line of creation is {}".format(
                                    #         #                             create_p))
                                    #         #                     if create_p:
                                    #         #                         company.quickbooks_last_sales_receipt_imported_id = int(
                                    #         #                             cust.get('Id'))
    ...
    def export_sale_order(self):
        sales = self.env['sale.order'].search([('state', 'in', ['done', 'sale'])])
        if not sales:
            raise UserError('There is no any record to be exported.')
    ...
    def export_invoice(self):
        invoices = self.env['account.move'].search([('state', '=', 'posted'), ('move_type', '=', 'out_invoice')])
        if not invoices:
            raise UserError('There is no any record to be exported.')
        for inv in invoices:
    ...
    def export_purchase_order(self):
        purchase = self.env['purchase.order'].search([('state', 'in', ['purchase', 'done'])])
        if not purchase:
            raise UserError('There is no any record to be exported.')
    ...
    def import_journal_entry_cron(self):
        ...
        company.import_journal_entry(call_from='cron')
        _logger.info("--------Journal Entry imported successfully.")
        self._cr.commit()

    def import_journal_entry(self, call_from=None):
        ...
        if company.access_token:
            ...
            if company.import_je_by_date:
                if self.journal_entry_import_by == 'crt_dt':
                    query = "select * from JournalEntry WHERE Metadata.CreateTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_je_date, company.quickbooks_last_journal_entry_imported_id)
                elif self.journal_entry_import_by == 'other_dt':
                    query = "select * from JournalEntry WHERE TxnDate >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_je_date, company.quickbooks_last_journal_entry_imported_id)
                else:
                    query = "select * from JournalEntry WHERE Metadata.LastUpdatedTime >= '%s' AND ID >= '%s' order by Id " % (
                        company.import_je_date, company.quickbooks_last_journal_entry_imported_id)
            else:
