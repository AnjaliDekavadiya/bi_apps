...
class ResPartner(models.Model):
    ...
    def create_partner(self, data, is_customer=False, is_vendor=False):
        ...
        for partner in partners:
            ...
            brw_partner = self.search(
                ['|', ('qbo_customer_id', '=', partner.get('Id')), ('name', '=', partner.get('DisplayName'))], limit=1)
            _logger.info("Browsing partner************ {}".format(brw_partner))
            if not brw_partner:
                _logger.info(
                    "Customer needs to be created$$$$$$$$$$$$$$$$$$${}".format(vals))
                brw_partner = self.search(
                     ['|', ('qbo_vendor_id', '=', partner.get('Id')), ('name', '=', partner.get('DisplayName'))], limit=1)
                if not brw_partner:
                    ...
                    if 'BillAddr' in partner and partner.get('BillAddr'):
                        ...
                        if 'Country' in partner.get('BillAddr'):
                            address_vals.update({
                                'state_id': self.env['res.country.state'].get_state_ref(partner.get('BillAddr').get('CountrySubDivisionCode'), partner.get('BillAddr').get('Country')) if 'Country' in partner.get(
    ...
    def export_single_Partner(self, partner):
        # _logger.info('\n\n000000000000000000000Inside export Single partner : '.format(partner))
        if partner.customer_rank:
