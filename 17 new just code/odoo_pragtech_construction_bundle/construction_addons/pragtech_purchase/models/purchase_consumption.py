...
class PurchaseConsumption(models.Model):
    ...
    purchase_ids = fields.Many2many('purchase.order', 'po_consumption_rel1', 'consumption_id', 'purchase_id', string='Purchase Orders')
    consumption_fulfill = fields.Boolean('Req fulfill', compute='is_consumption_fulfill')
    stage_id = fields.Many2one('stage.master', 'Stage', default=_default_stage, readonly=True, copy=False, track_visibility='onchange')
    ...
    material_category = fields.Many2one('product.category', string='Material Category', related='material_id.categ_id', store=True)
    related_task_category = fields.Many2one('task.category', related='task_id.category_id', store=True)
    task_category = fields.Many2many('task.category', 'purchase_cons_req_task_cat_rel', 'purchase_req_id', 'task_category_id',
        ...
        if self.counter == 0:
            ...
            if context.get('copy'):
                ...
                if self.product_location_id:
                    ...
                    project_location = self.env['stock.location'].search([('project_id', '=', self.project_id.id)], limit=1)
                    loc_id = False
                    loc_dest_id = False
                    ...
                    location_ids = self.env['stock.quant'].search([('product_id.id', '=', self.material_id.id), ('location_id', '=', self.product_location_id.id)])
