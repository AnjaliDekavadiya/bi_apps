...
class VendorQuotation(models.Model):
    ...
    date_order = fields.Datetime('Quotation Date', required=True, select=True, copy=False, default=fields.Datetime.now(),
                                 help='Depicts the date where the Quotation should be validated and converted into a purchase order.', readonly="state == 'confirm'")
    partner_id = fields.Many2one('res.partner', string='Vendor', required=True, change_default=True, track_visibility='always', readonly="state == 'confirm'")
    partner_ref = fields.Char('Reference', copy=False, readonly="state == 'confirm'", help="Reference of the sales order or bid sent by the vendor. "
    ...
    order_line = fields.One2many('vendor.quotation.line', 'order_id', string='Order Lines', copy=True, readonly="state == 'confirm'")
    ...
    company_id = fields.Many2one('res.company', 'Company', index=1, default=lambda self: self.env.user.company_id.id, readonly="state == 'confirm'")
...
class VendorQuotationLine(models.Model):
    ...
    taxes_id = fields.Many2many('account.tax', 'vendor_quot_line__tax_rel', 'tax_id', 'vendor_quot_line_id', string='Taxes')
    ...
    order_id = fields.Many2one('vendor.quotation', string='Order Reference', select=True, required=True, ondelete='cascade')
