...
class PurchaseOrder(models.Model):
    ...
    material_category = fields.Many2many('product.category', 'po_category_rel', 'purchase_order_id', 'category_id', string='Material Category')
    ...
    order_line = fields.One2many('purchase.order.line', 'order_id', string='Order Lines', readonly="state != 'purchase' or state != 'done' or state != 'cancel'", copy=True, ondelete='cascade', store=True)
    ...
    def custom_compute_qty_received(self, product_id):
        total = 0
        picking = self.env['stock.picking'].search([('state', '=', 'done'), ('origin', '=', self.name), ('partner_id', '=', self.partner_id.id)])
    ...
    def compute_po_lines(self, purchase_order_id, project_wbs, project):
        ...
        for line in compressed_material_list:
            ...
            project_wbs_obj = self.env['project.task'].search([('name', '=', project_wbs.name), ('project_id', '=', project.id), ('sub_project', '=', self.sub_project.id)])
    ...
    def custom_create_picking(self):
        for order in self:
            if any([ptype in ['product', 'consu'] for ptype in order.order_line.mapped('product_id.type')]):
                ...
                moves = order.order_line.filtered(lambda r: r.product_id.type in ['product', 'consu']).custom_create_stock_moves(picking)
    ...
    def cron_custom_create_picking(self):
        for order in self:
            if any([ptype in ['product', 'consu'] for ptype in order.order_line.mapped('product_id.type')]):
                res = order._prepare_picking()
                picking = self.env['stock.picking'].create(res)
                moves = order.order_line.filtered(lambda r: r.product_id.type in ['product', 'consu']).cron_custom_create_stock_moves(picking)
    ...
    def onchange_requisition_ids(self):
        ...
        if self.requisition_ids:
            for requisition in self.requisition_ids:
                purchase_line_obj = self.env['purchase.order.line'].search([('order_id.state', 'in', ['done', 'purchase']), ('requisition_id', '=', requisition.id)])
...
class PurchaseOrderLine(models.Model):
    ...
    total_rate = fields.Float('Total Rate', help='This is rate of product including transportation and other charges', compute='get_landed_cost', store=True)
    ...
    def custom_compute_qty_received(self, product_id):
        total = 0
        picking = self.env['stock.picking'].search([('state', '=', 'done'), ('po_id', '=', self.order_id.id), ('partner_id', '=', self.order_id.partner_id.id)])
        for pickings in picking:
            for line in pickings.pack_operation_product_ids:
                if line.product_id.id == product_id:
...
class POMaterialRateLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            taxes = line.tax_id.compute_all(price, line.currency_id, quantity=1.0, product=line.name, partner=line.supplier_id)
            line.update({
