...
class QuotationComparison(models.Model):
    ...
    quotation_details = fields.One2many('quotation.details', 'quotation_comp_detail_id', string='Quotation Lines', copy=True)
    quotation_comp_particular = fields.One2many('quotation.compare.particular', 'quotation_comp_pert_id', string='Quotation Details Line', copy=True)
    total_amount = fields.Float(string='Total Amount', store=True, compute='compute_total_amount')

    quotation_comp_vendor1 = fields.One2many('quotation.compare.vendor1', 'quotation_comp_ven1_id', string='Quotation Details Vendor1', copy=True)
    quotation_comp_vendor2 = fields.One2many('quotation.compare.vendor2', 'quotation_comp_ven2_id', string='Quotation Details Vendor2', copy=True)
    quotation_comp_vendor3 = fields.One2many('quotation.compare.vendor3', 'quotation_comp_ven3_id', string='Quotation Details Vendor3', copy=True)
    quotation_comp_vendor4 = fields.One2many('quotation.compare.vendor4', 'quotation_comp_ven4_id', string='Quotation Details Vendor4', copy=True)
    quotation_comp_vendor5 = fields.One2many('quotation.compare.vendor5', 'quotation_comp_ven5_id', string='Quotation Details Vendor6', copy=True)
    quotation_comp_vendor6 = fields.One2many('quotation.compare.vendor6', 'quotation_comp_ven6_id', string='Quotation Details Vendor6', copy=True)

    quotation_comp_tax_particular = fields.One2many('tax.particular', 'quotation_comp_taxper_id', string='Quotation Tax Particular Details', copy=True)

    quotation_comp_tax_details1 = fields.One2many('tax.details.vendor1', 'quotation_comp_tax1_id', string='Quotation Tax1 Details', copy=True)
    quotation_comp_tax_details2 = fields.One2many('tax.details.vendor2', 'quotation_comp_tax2_id', string='Quotation Tax2 Details', copy=True)
    quotation_comp_tax_details3 = fields.One2many('tax.details.vendor3', 'quotation_comp_tax3_id', string='Quotation Tax3 Details', copy=True)
    quotation_comp_tax_details4 = fields.One2many('tax.details.vendor4', 'quotation_comp_tax4_id', string='Quotation Tax4 Details', copy=True)
    quotation_comp_tax_details5 = fields.One2many('tax.details.vendor5', 'quotation_comp_tax5_id', string='Quotation Tax5 Details', copy=True)
    quotation_comp_tax_details6 = fields.One2many('tax.details.vendor6', 'quotation_comp_tax6_id', string='Quotation Tax6 Details', copy=True)
    ...
    def confirm_action(self):
        ...
        no_of_approve = []
        no_of_products = [particular.product_id for particular in self.quotation_comp_particular if particular.is_approve]
    ...
    def onchange_project_wbs1(self):
        ...
        # Set vendor list
        quotation_lines = self.env['vendor.quotation.line'].search([('product_id', 'in', material_ids), ('order_id.date_order', '>=', self.from_date), ('order_id.date_order', '<=', self.to_date)])
    ...
    @api.depends('quotation_comp_vendor1.amount', 'quotation_comp_vendor2.amount', 'quotation_comp_vendor3.amount',
                 ...
                 'quotation_comp_tax_details1.tax', 'quotation_comp_tax_details2.tax', 'quotation_comp_tax_details3.tax',
    ...
    def compute_quotation_details(self):
        ...
        quotation_ids = self.env['vendor.quotation'].search([('date_order', '>=', self.from_date), ('state', '=', 'confirm'),
            ...
            for vendor in self.quotation_comp_vendor1:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        ...
                        record = self.env['tax.details.vendor1'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax1_id', '=', self.id)])
                        record.write({
        ...
        if self.quotation_comp_vendor2:
            ...
            for vendor in self.quotation_comp_vendor2:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        record = self.env['tax.details.vendor2'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax2_id', '=', self.id)])
                        record.write({
                            'tax': tax_amount[tax_line],
                        })
                    else:
        ...
        if self.quotation_comp_vendor3:
            ...
            for vendor in self.quotation_comp_vendor3:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        record = self.env['tax.details.vendor3'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax3_id', '=', self.id)])
                        record.write({
                            'tax': tax_amount[tax_line],
                        })
                    else:
        ...
        if self.quotation_comp_vendor4:
            ...
            for vendor in self.quotation_comp_vendor4:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        record = self.env['tax.details.vendor4'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax4_id', '=', self.id)])
                        record.write({
                            'tax': tax_amount[tax_line],
                        })
                    else:
        ...
        if self.quotation_comp_vendor5:
            ...
            for vendor in self.quotation_comp_vendor5:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        record = self.env['tax.details.vendor5'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax5_id', '=', self.id)])
                        record.write({
                            'tax': tax_amount[tax_line],
                        })
                    else:
        ...
        if self.quotation_comp_vendor6:
            ...
            for vendor in self.quotation_comp_vendor6:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        record = self.env['tax.details.vendor6'].search([('tax_id', '=', tax_line.id), ('quotation_comp_tax6_id', '=', self.id)])
                        record.write({
                            'tax': tax_amount[tax_line],
                        })
                    else:
        ...
        if self.quotation_details:
            ...
            for line in self.quotation_details:
                ...
                if line.is_use:
                    ...
                    material_lines = self.env['task.material.line'].search([('material_id', '=', line.product_id.id), ('wbs_id', '=', self.project_wbs_id.id)])
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[0]:
                        ...
                        blank_line1 = self.env['quotation.compare.vendor1'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven1_id', '=', self.id)])
                        if not blank_line1:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[1]:
                        ...
                        blank_line2 = self.env['quotation.compare.vendor2'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven2_id', '=', self.id)])
                        if not blank_line2:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[2]:
                        ...
                        blank_line3 = self.env['quotation.compare.vendor3'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven3_id', '=', self.id)])
                        if not blank_line3:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[3]:
                        ...
                        blank_line4 = self.env['quotation.compare.vendor4'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven4_id', '=', self.id)])
                        if not blank_line4:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[4]:
                        ...
                        blank_line5 = self.env['quotation.compare.vendor5'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven5_id', '=', self.id)])
                        if not blank_line5:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
            ...
            for k_parent, v_parent in quotation_details_dict.items():
                for k, v in v_parent.items():
                    if k == selected_vendor_ids[5]:
                        ...
                        blank_line6 = self.env['quotation.compare.vendor6'].search([('product_id', '=', line.product_id.id), ('vendor_id', '=', None), ('quotation_comp_ven6_id', '=', self.id)])
                        if not blank_line6:
                            vals = {
                                'product_id': line.product_id.id,
                                'vendor_id': None,
...
class QuotationDetails(models.Model):
    ...
    quotation_details_ = fields.One2many('quotation.compare.particular', 'quotation_comp_pert_detail_id', string='Quotation Details Line')
    quotation_details_ven1 = fields.One2many('quotation.compare.vendor1', 'quotation_comp_ven1_detail_id', string='Quotation Details Vendor1')
    quotation_details_ven2 = fields.One2many('quotation.compare.vendor2', 'quotation_comp_ven2_detail_id', string='Quotation Details Vendor2')
    quotation_details_ven3 = fields.One2many('quotation.compare.vendor3', 'quotation_comp_ven3_detail_id', string='Quotation Details Vendor3')
    quotation_details_ven4 = fields.One2many('quotation.compare.vendor4', 'quotation_comp_ven4_detail_id', string='Quotation Details Vendor4')
    quotation_details_ven5 = fields.One2many('quotation.compare.vendor5', 'quotation_comp_ven5_detail_id', string='Quotation Details Vendor5')
    quotation_details_ven6 = fields.One2many('quotation.compare.vendor6', 'quotation_comp_ven6_detail_id', string='Quotation Details Vendor6')

    quotation_tax_per = fields.One2many('tax.particular', 'quotation_comp_taxper_detail_id', string='Quotation Particular Tax')

    quotation_tax1_detail_per = fields.One2many('tax.details.vendor1', 'quotation_comp_tax1_detail_id', string='Quotation Detail Tax1')
    quotation_tax2_detail_per = fields.One2many('tax.details.vendor2', 'quotation_comp_tax2_detail_id', string='Quotation Detail Tax2')
    quotation_tax3_detail_per = fields.One2many('tax.details.vendor3', 'quotation_comp_tax3_detail_id', string='Quotation Detail Tax3')
    quotation_tax4_detail_per = fields.One2many('tax.details.vendor4', 'quotation_comp_tax4_detail_id', string='Quotation Detail Tax4')
    quotation_tax5_detail_per = fields.One2many('tax.details.vendor5', 'quotation_comp_tax5_detail_id', string='Quotation Detail Tax5')
    quotation_tax6_detail_per = fields.One2many('tax.details.vendor6', 'quotation_comp_tax6_detail_id', string='Quotation Detail Tax6')
...
class TaxParticular(models.Model):
    ...
    quotation_comp_taxper_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
