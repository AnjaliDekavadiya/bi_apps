...
class PurchaseConsumptionWizard(models.TransientModel):
    ...
    sub_project = fields.Many2one('sub.project', 'Sub Project', required=True,domain=[('approval_state','=','approve')])
    ...
    def task_category_onchange(self):
        return {
            'domain': {'task_id': [('category_id', '=', self.task_category.id), ('is_task', '=', True), ('project_wbs_id', '=', self.name.id)]}
        }

    def get_is_red(self, material_id, partner_id):
        product_id = self.env['product.product'].search([('id', '=', material_id)])
        price_info = self.env['product.supplierinfo'].search([('product_tmpl_id', '=', product_id.product_tmpl_id.id), ('is_active', '=', True), ('name', '=', partner_id)])
    ...
    def compute_consumptions_lines(self):
        ...
        for line in material_estimate_obj:
            location_ids = self.env['stock.quant'].search([('product_id.id', '=', line.material_id.id), ('location_id.usage', '=', 'internal')], limit=1)

            consumption_records = self.env['purchase.consumption'].search([('material_id', '=', line.material_id.id), ('project_id', '=', self.project_id.id),
                                                                           ('sub_project', '=', self.sub_project.id), ('project_wbs', '=', self.name.id), ('state', '=', 'confirm')])
    ...
    def search_consumptions_on_po(self):
        ...
        for line in purchase_consumption_obj:
            product_supplier_obj = self.env['product.supplierinfo'].search([('product_name', '=', line.material_id.name), ('is_active', '=', True), ('name', '=', Purchase_order_obj.partner_id.id)])
    ...
    def add_consumption(self):
        Purchase_order_obj = self.env['purchase.order'].browse(self._context.get('active_id'))
        po_consumption_line_obj = self.env['purchase.consumption'].search([('order_id', '=', self._context.get('active_id'))])
        ...
        for line in self.consumption_ids:
            if line.is_use:
                ...
                if line.consumption_name not in old_po_consumption_lines and line.current_order_qty > 0 and line.current_order_qty <= (line.consumption_qty - line.total_ordered_qty):
                    vals = {
                        'project_id': self.project_id.id,
                        'project_wbs': self.name.id,
                        'sub_project': self.sub_project.id,
