...
class PurchaseRequisitionWizard(models.TransientModel):
    ...
    material_category = fields.Many2many('product.category', 'wizard_prod_cat_rel', 'wizard_id', 'cat_id')
    ...
    def get_is_red(self, material_id, partner_id):
        price_info = self.env['product.supplierinfo'].search([('product_tmpl_id', '=', material_id), ('is_active', '=', True), ('partner_id', '=', partner_id)])
    ...
    def create_requisitions(self):
        ...
        view_id = self.env.ref('pragtech_purchase.purchase_requisitions_tree_view').id
    ...
    def search_requisitions_on_po(self):
        ...
        for line in purchase_requisition_obj:
            product_supplier_obj = self.env['product.supplierinfo'].search([('product_name', '=', line.material_id.name), ('is_active', '=', True), ('partner_id', '=', Purchase_order_obj.partner_id.id)])
...
class PurchaseRequisitionWizardLine(models.TransientModel):
    ...
    material_category = fields.Many2many('product.category', 'prod_cat_wizard_line_rel', 'wizard_id', 'cat_id', related='order_id.material_category')
