...
from odoo.addons.website_sale.controllers.main import WebsiteSale as WSale
...
class ProductShipping(http.Controller):
    ...
    def sol_update_website_sale_delivery_return(self, order, carrier_id, lines):
        ...
        monetary_options = {
            'display_currency': request.website._get_current_pricelist().currency_id,
        }
        if order:
...
class WebsiteSaleDelivery(WSale):
