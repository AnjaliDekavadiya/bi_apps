...
class ReturnOrder(models.Model):
    ...
    _name = 'return.order'
    _inherit = ['return.order','portal.mixin']
    ...
    is_scrap = fields.Boolean(
        string="Is Scrap",
        compute='_compute_replacement_custom',
        store=True
    )
    is_replacement = fields.Boolean(
        string="Is Replacement",
        compute='_compute_replacement_custom',
        store=True
    )
    ...
    def _compute_access_url(self):
        ...
        for rec in self:
            rec.access_url = '/my/returns/printrma/%s' % (rec.id)

    @api.depends('return_product_line_ids',
        'return_product_line_ids.repair_scrape')
    def _compute_replacement_custom(self):
        for rec in self:
...
#    @api.multi odoo13
    def send_rma(self):
        ...
        try:
            ...
            template_id = self.env['ir.model.data']._xmlid_to_res_id('website_request_return_rma_odoo.email_template_rma_sendto_customer', raise_if_not_found=False)
        except ValueError:
            template_id = False
        try:
