...
class Jira_Config_Settings(models.Model):
    ...
    account_id = fields.Char(string='Jira Account')
    ...
    def post_file(self, request, filename, filepath):
        ...
        try:
            ...
            response = requests.post(self.jira_url + '/rest/api/latest/' + request,
                                     auth=(self.jira_login, self.password),
    ...
    def check_response(self, response):
        ...
        if response.status_code not in [200, 201, 204, 404]:
            try:
                ...
                raise UserError(_('Response status code:  ' + str(response.status_code)))
            error_msg = ''
    ...
    def constrains_jira_url(self):
        ...
        if not self.jira_url.startswith('http://') and not self.jira_url.startswith('https://'):
            raise UserError(_('Url must start with http:// or https://'))
