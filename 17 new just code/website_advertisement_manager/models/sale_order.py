...
class SaleOrder(models.Model):
    ...
    def _cart_update(self, product_id, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if order_line and product and product.is_ad_block:
            order = order_line.order_id
            
            if res.get('quantity') and res.get('quantity') >=0:
            ...
            if ad_vals:
                order_line.write({
                    ...
                    'ad_date_to' : ad_vals.get("ad_date_from"),
                    'ad_date_from' : ad_vals.get("ad_date_to")
                })
    ...
    def _prepare_order_line_values(
        ...
        **kwargs
        ...
        self.ensure_one()
        res = super(SaleOrder, self)._prepare_order_line_values(product_id, quantity, 
                                        linked_line_id,no_variant_attribute_values,product_custom_attribute_values,**kwargs)
        product = self.env['product.product'].browse(product_id)
...
class SaleOrderLine(models.Model):
    ...
    def _compute_ad_block_status(self):
        for rec in self:
            ...
            if rec.is_ad_block_line and rec.state in ['sale','done']:
                if date_today >= ad_date_from and date_today <= ad_date_to:
    ...
    def _check_block_dates_availability(self, vals):
        ...
        if product_id and product_id.is_ad_block:
            ...
            order_lines = order_lines.filtered(lambda l: WebsiteAdvertisement().check_sol_paid(l))
            block_date_list = []
    ...
    def send_ad_block_status_update_mail(self):
        # if self.env['ir.default'].sudo()._get('res.config.settings', 'auto_approve_ad_block'):
    ...
    def ad_content_auto_approve(self):
        for rec in self:
            ad_block_auto_approve = self.env['ir.default'].sudo()._get('res.config.settings', 'auto_approve_ad_block')
