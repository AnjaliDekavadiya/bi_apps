...
class PortalAccount(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        my_ad_blocks_count = request.env['sale.order.line'].sudo().search_count([
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'my_ad_blocks_count' in counters:
            values['my_ad_blocks_count'] = SaleOrderLine.search_count(self._prepare_wk_advertisement_domain(partner)) \
                if SaleOrderLine.check_access_rights('read', raise_exception=False) else 0
        return values

    def _prepare_wk_advertisement_domain(self, partner):
        return [
            ('order_partner_id', '=', partner.id),
            ('is_ad_block_line', '=', True)
        ]

    def _get_ad_block_searchbar_sortings(self):
        return {
    ...
    def portal_my_ad_block_detail(self, ad_block_id=None, access_token=None, **kw):
        ad_b = request.env['sale.order.line'].sudo().browse([ad_block_id])
        if not ad_b.exists():
