...
class WebsiteAdvertisement(http.Controller):
    ...
    def check_sol_paid(self,line):
        return True if line.state in ['sale','done'] else False
    ...
    def book_ad_block(self, block_id):
        ...
        order_lines = order_lines.filtered(lambda l: self.check_sol_paid(l))
        block_date_list = []
        if order_lines and order_lines.exists():
        ...
        values['website_advertisement_manager.website_ad_block_book_modal'] = request.env['ir.ui.view']._render_template("website_advertisement_manager.website_ad_block_book_modal", values)
        return values
    ...
    def validate_ad_dates(self, block_id, ad_date_from, ad_date_to, **kw):
        ...
        try:
            ad_date_from = datetime.strptime(ad_date_from,"%Y-%m-%d").date()
            ad_date_to = datetime.strptime(ad_date_to,"%Y-%m-%d").date()
        ...
        order_lines = request.env['sale.order.line'].sudo().search([('product_id','=', block_id.id),
            ('is_ad_block_line','=',True),('ad_block_status', '!=', 'expire'),('order_id.state','=','sale')])
        order_lines = order_lines.filtered(lambda l: self.check_sol_paid(l))
        block_date_list = []
        if order_lines:
            for line in order_lines:
...
class AdvertiseWebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if product_id and product_id.is_ad_block:
            ad_vals = {
                'ad_date_to' : datetime.strptime(kw.get("ad_date_to"),"%Y-%m-%d").date(),
                'ad_date_from' : datetime.strptime(kw.get("ad_date_from"),"%Y-%m-%d").date(),
            }
