...
class RmaRma(models.Model):
	...
	def _set_inv_created(self):
		for obj in self:
			if obj.refund_invoice_id:
				...
				if obj.refund_invoice_id.state != 'paid':
	...
	def create(self,vals):
		obj_rma = super(RmaRma, self).create(vals)
		if obj_rma.marketplace_seller_id:
			resConfig = self.env['res.config.settings']
			enable_seller_template = resConfig.get_mp_global_field_value("enable_notify_seller_4_rma")
			if enable_seller_template:
				temp = resConfig.get_mp_global_field_value("notify_seller_for_rma_creation")
				temp_id = self.env['mail.template'].browse(temp)
				if temp_id:
					temp_id.send_mail(obj_rma.id, force_send=True)
				return obj_rma
		else:
			return obj_rma
