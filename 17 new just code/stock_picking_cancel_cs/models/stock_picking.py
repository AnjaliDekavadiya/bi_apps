            ...
            if self.env.context.get('Flag',False) and picking.state =='done':
                ...
                for move in account_moves:
                    ...
                    try:
                        ...
                        landed_cost_rec= landed_obj.search([('picking_ids', '=', picking.id), ('state', '=', 'done')])
                    except :
                        pass

                    if landed_cost_rec:           
                        raise ValidationError('This Delivery is set in landed cost record %s you need to delete it fisrt then you can cancel this Delivery'%','.join(landed_cost_rec.mapped('name')))
                    ...
                    if move.state == "done" and move.product_id.type == "product":
                        for move_line in move.move_line_ids:
                            ...
                            quant_obj._update_available_quantity(move_line.product_id, move_line.location_id, quantity, move_line.lot_id.id)
                            quant_obj._update_available_quantity(move_line.product_id, move_line.location_dest_id, quantity * -1, move_line.lot_id.id)
