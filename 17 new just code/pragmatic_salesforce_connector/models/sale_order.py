    ...
    def sendDataToSf(self, order_dict, is_cron=False):
        ...
        if sf_access_token:
            ...
            if self.sale_order_salesforce_id:
                ...
                res = requests.request('PATCH', sf_config.sf_url + endpoint + '/' + self.sale_order_salesforce_id, headers=headers, data=payload)
            ...
            if self.order_line:
                for line in self.order_line:
                    if line.sale_order_line_salesforce_id and line.sale_order_line_updated:
                        delete_so_line = self.delete_sale_order_line_to_salesforce(line, line.sale_order_line_salesforce_id, is_cron=is_from_cron)
                        if delete_so_line:
                            ...
                            order_data = requests.request('GET',
                                                          sf_config.sf_url + "/services/data/v40.0/query/?q=select Pricebook2Id from Order where Id = '{}'".format(self.sale_order_salesforce_id),
                                                          headers=headers)
                            if order_data.text:
                        ...
                        salesforce_pbe = ''
                        pricebook_id = ''
                        order_data = requests.request('GET', sf_config.sf_url + "/services/data/v40.0/query/?q=select Pricebook2Id from Order where Id = '{}'".format(self.sale_order_salesforce_id),
                                                      headers=headers)
                        if order_data.text:
                            order_data = json.loads(str(order_data.text))
                            pricebook_id = order_data.get('records')[0].get('Pricebook2Id')
...
class SaleOrderLine(models.Model):
    ...
    def unlink(self):
        ...
        if sf_config.sf_access_token:
            ...
            for record in self:
                if (record.x_salesforce_id or record.sale_order_line_salesforce_id) and (record.order_id.state == 'draft' or record.order_id.state == 'cancel'):
                    ...
                    if res.status_code == 404 or res.status_code == 400:
                        endpoint = "/services/data/v52.0/sobjects/OrderItem/" + str(record.sale_order_line_salesforce_id)
                        res = requests.request('DELETE', sf_config.sf_url + endpoint, headers=headers)
                        if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
