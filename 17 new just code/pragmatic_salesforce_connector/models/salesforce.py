...
class SalesForceMaster(models.Model):
    ...
    def sf_sendMessage(self, message):
        view_ref = self.env['ir.model.data']._xmlid_to_res_id('pragmatic_salesforce_connector.salseforce_message_wizard_form')
        if view_ref:
            return {
        ...
        try:
            if self.sf_access_token:
                ...
                if product_partner != False and not type(product_partner) == type(dummy_dict) and is_account == False:
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Contact where Id='{}' LIMIT {}".format(product_partner,
                                                                                                                        self.import_limit)
                    else:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Contact where Id='{}'".format(product_partner)
                    data = requests.request('GET', self.sf_url + endpoint, headers=headers)
                elif is_account and product_partner != False and not type(product_partner) == type(dummy_dict):
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account where Id='{}' LIMIT {}".format(product_partner,
                                                                                                                        self.import_limit)
                    else:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account where Id='{}'".format(product_partner)

                    data = requests.request('GET', self.sf_url + endpoint, headers=headers)
                elif is_account:
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account LIMIT {}".format(product_partner, self.import_limit)
                    else:
    ...
    def getSfProductPrice(self, sf_id):
        if self.sf_access_token:
            ...
            query_url = ""
            if self.import_limit:
                query_url = "/services/data/v40.0/query/?q=select UnitPrice from pricebookentry where Product2Id='{}' LIMIT {}".format(str(sf_id),
                                                                                                                                       self.import_limit)
            else:
                query_url = "/services/data/v40.0/query/?q=select UnitPrice from pricebookentry where Product2Id='{}'".format(str(sf_id))
                                        ...
                                        if quote_dict:
                                            if 'Tax' in quote_dict and 'Subtotal' in quote_dict:
                                                if quote_dict['Tax'] and quote_dict['TotalPrice']:
                                                    ...
                                                    calculated_tax = round(((quote_dict['Tax'] / order_total_price) * 100), 7)
                                                    if calculated_tax:
                                                        account_tax = self.env['account.tax'].sudo().search(
                                                            [('price_include', '=', False), ('amount', '=', calculated_tax)], limit=1)
                                                        if account_tax:
                        ...
                        if recs.get('records'):
                            for r in recs.get('records'):
                                ...
                                if quote_data:
                                    ...
                                    ''' GET QUOTE LINES '''
                                    quote_line_data = self.getSfQuoteLineData(quote_id=r.get('Id'), quote_dict=quote_data, is_cron=is_from_cron)
                                    if quote_line_data:
                                        for quote_line in quote_line_data:
                                            sf_quotation_line_list.append(quote_line.get('Id'))
                                    ''' GET CUSTOMER '''
                                    if quote_data.get('OpportunityId') and not quote_data.get('ContactId') and not quote_data.get(
                                        ...
                                        raise UserError(_("Please add Account Id for %s Opportunity in Salesforce", quote_data.get('OpportunityId')))

                                    if quote_data.get('OpportunityId'):
                                        lead_id = self.env['crm.lead'].sudo().search(
                                            [('type', '=', 'opportunity'), ('x_salesforce_id_oppo', '=', quote_data.get('OpportunityId'))])
                                        if lead_id:
                                    ...
                                    if quote_dict:
                                        ...
                                        sale_order_id = self.env['sale.order'].sudo().search(
                                            [('x_salesforce_id', '=', quote_data.get('Id')), ('state', '=', 'draft')], limit=1)
                                        if sale_order_id:
                                            sale_order_line_ids = self.env['sale.order.line'].sudo().search([('order_id', '=', sale_order_id.id)])
                ...
                if data.text:
                    ...
                    if recs:
                        ...
                        if recs.get('ContractId'):
                            contract_id = self.env['sf.contract'].sudo().search([('x_salesforce_id', '=', recs.get('ContractId'))])
                            if contract_id:
                        ...
                        for rec in parsed_data.get('records'):
                            ...
                            if so_data:
                                ...
                                if rec.get('Status') != 'Draft':
                                    so_data['state'] = 'sale'
                                ''' GET SO LINES '''
                                so_line_data = self.getSfQuoteLineData(is_so=True, so_id=str(rec.get('Id')), quote_dict=so_data, is_cron=is_from_cron)
                                if so_line_data:
                                    for so_line_sf_id in so_line_data:
                                        sf_sale_order_line2.append(so_line_sf_id.get('Id'))
                                # Check if quotation dosent exists
                                so_exists = self.env['sale.order'].search([('sale_order_salesforce_id', '=', rec.get('Id'))], limit=1)
                                if not so_exists:
                                        ...
                                        if so_line_data:
                                            for soline in so_line_data:
                                                ...
                                                if not so_line_exists:
                                                    ...
                                                    if so_line_create:
                                                        so_line_create.sudo().write({'sale_order_line_salesforce_id': salesforce_sale_order_id})
    ...
    def create_odoo_sf_contact_dictionary(self, sf_contact_info):
        ...
        if sf_contact_info.get('MailingCountry', False):
            if sf_contact_info.get('MailingCountry') in ['usa', 'USA', 'us', 'US', 'United States of America', 'united states of america',
                                                         'united states', 'United States']:
                country_id = self.env['res.country'].search([('code', 'like', 'US')], limit=1)
                partner_dict['country_id'] = country_id.id
            else:
                country_id = self.env['res.country'].search(
                    ['|', ('name', '=', sf_contact_info.get('MailingCountry')), ('code', '=', sf_contact_info.get('MailingCountry'))], limit=1)
                if country_id:
                    partner_dict['country_id'] = country_id.id
                else:
        ...
        if sf_contact_info.get('MailingState', False):
            ...
            if country_id:
                ...
                state_id = self.env['res.country.state'].search(['|', ('name', '=', sf_state), ('code', '=', sf_state)], limit=1)
            if state_id:
                            ...
                            for rec in recs:
                                ...
                                if activity_read.get('WhoId'):
                                    ...
                                    # activity_dict['res_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            ...
                                            _logger.error("WhoId Not found in the record please add it first for id " + activity_read.get('Id'))
                                else:
                                    if not is_from_cron:
                                        raise UserError(_("WhoId Not found in the record please add it first for id " + activity_read.get('Id')))
                                    else:
                                ...
                                if activity_read.get('WhatId'):
                                    ...
                                    # activity_dict['parent_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            raise UserError(
                                                _("WhatId Not found in the record please add it first for id1 " + activity_read.get('Id')))
                                        else:
                                            _logger.error("WhatId Not found in the record please add it first for id1 " + activity_read.get('Id'))
                                else:
                                    if not is_from_cron:
                                        raise UserError(_("WhatId Not found in the record please add it first for id2 " + activity_read.get('Id')))
                                    else:
                                        _logger.error("WhatId Not found in the record please add it first for id2 " + activity_read.get('Id'))
                                ...
                                if activity_read.get('WhoId') and activity_read.get('WhatId'):
                                    ...
                                    # activity_dict['res_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            raise UserError(
                                                _("WhatId Not found in the record please add it first for id3 " + activity_read.get('Id')))
                                        else:
                                            _logger.error("WhatId Not found in the record please add it first for id3 " + activity_read.get('Id'))
                                else:
                                    if not is_from_cron:
                                        raise UserError(_("WhatId Not found in the record please add it first for id4 " + activity_read.get('Id')))
                                    else:
                                        _logger.error("WhatId Not found in the record please add it first for id4 " + activity_read.get('Id'))
