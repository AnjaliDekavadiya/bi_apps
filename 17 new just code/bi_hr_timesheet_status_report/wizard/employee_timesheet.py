...
class Employees_Timesheet_Reports(models.TransientModel):
    ...
    def incomplete_sheets_cron(self):
        ...
        company_obj = self.env['res.users'].browse(self.env['res.users'].self.env.user.id).company_id
        send_to_list = []
        ...
        result = self.env['ir.actions.report']._render_qweb_pdf("bi_hr_timesheet_status_report.report_employee_timesheet_print", a.id)[0]
        ...
        values = email_template_obj._generate_template([self.id],('subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'))[self.id]
        values['email_from'] = self.env['res.users'].browse(self.env['res.users'].self.env.user.id).partner_id.email
        values['email_to'] = str(emal_to)
        values['res_id'] = self.id
        values['attachment_ids'] = [(6,0,[attachment.id])]
        values['author_id'] = self.env['res.users'].browse(self.env['res.users'].self.env.user.id).partner_id.id
...
class Employee_incomplete_timesheet(models.TransientModel):
    ...
    _description="Incomplete Timesheet"
