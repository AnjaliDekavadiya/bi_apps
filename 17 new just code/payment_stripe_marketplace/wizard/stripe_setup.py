...
class StripeMarketplaceSetup(models.TransientModel):
    ...
    def integrating_stripe_oauth(self):
        ...
        if not payment_provider:
            raise UserError(
                _("Stripe marketplace or stripe client key not found. Please configure the Stripe Payment Method."))
        return {
        ...
        if stripe_user_id:
            if payment_provider and payment_provider.stripe_mp_client_key:
                ...
                r = requests.post(deauth_url,
                    auth=(payment_provider.stripe_secret_key, ''),
                    params={"client_id":str(payment_provider.stripe_mp_client_key), "stripe_user_id":stripe_user_id})
