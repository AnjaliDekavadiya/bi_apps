...
class StripeController(StripeController):
    ...
    def stripe_return_from_checkout(self, **data):
        ...
        # Retrieve the tx and provider based on the tx reference included in the return url
        ...
        payment_intent = provider_sudo._stripe_make_request(
        ...
        request.env['payment.transaction'].sudo()._handle_notification_data('stripe', data)
    ...
    def _generate_and_pay_seller_payment_by_sripe_mp(self, tx, seller_bill):
        if seller_bill:
            _logger.info('<%s> transaction completed, auto-generated seller bill %s (ID %s) for %s (ID %s)',
                         tx.provider_id.name, seller_bill.name, seller_bill.id, tx.sale_order_ids.name, tx.sale_order_ids.id)
            seller_journal_id = request.env['res.config.settings'].get_mp_global_field_value("seller_payment_journal_id")
            journal_obj = request.env["account.journal"].sudo().browse(seller_journal_id) or tx.provider_id.journal_id
            if journal_obj:
                seller_bill.action_post()
        else:
            _logger.warning('<%s> transaction completed, could not auto-generate seller bill for %s (ID %s)',
                            tx.provider_id.name, tx.sale_order_id.name, tx.sale_order_id.id)
    ...
    def stripe_mp_authorize(self, **post):
        payment_provider = request.env["payment.provider"].sudo().search(
            [('code', '=', 'stripe'),("is_mp_stripe", "=", True),('state', '!=', 'disabled')], limit=1)
        if payment_provider and payment_provider.stripe_mp_client_key:
            base_url = request.env['ir.config_parameter'].sudo().get_param('web.base.url')
            if payment_provider.stripe_account_type == "express":
            ...
            url = url_type + "?response_type=code&client_id=" + \
                str(payment_provider.stripe_mp_client_key) + "&scope=read_write"
            if base_url:
    ...
    def stripe_mp_aouth_return(self, **post):
        ...
        if code:
            data = {
                'client_secret': payment_provider.stripe_secret_key,
                'grant_type': 'authorization_code',
                'client_id': payment_provider.stripe_mp_client_key,
                'code': code
            }
        ...
        stripe_mp_setup_menu = request.env['ir.model.data']._xmlid_lookup(
            'payment_stripe_marketplace.stripe_marketplace_setup_menu')[1]
        stripe_mp_setup_action = request.env['ir.model.data']._xmlid_lookup(
            'payment_stripe_marketplace.stripe_marketplace_setup_action')[1]
